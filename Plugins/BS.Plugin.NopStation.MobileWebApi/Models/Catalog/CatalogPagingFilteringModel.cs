    ...
    public partial class CatalogPagingFilteringModel : BasePageableModel
    {
        ...
        public partial class SpecificationFilterModel : BaseNopModel
        {
            ...
            public virtual void PrepareSpecsFilters(IList<int> alreadyFilteredSpecOptionIds,
                ...
                IProductAttributeService productAttributeService, IList<ProductOverViewModelApi> products)
            {
                ...
                foreach (var sao in specificationAttributeOptions)
                {
                    ...
                    if (sa != null)
                    {
                        allFilters.Add(new SpecificationAttributeOptionFilter
                        {
                            ...
                            SpecificationAttributeName = _localizationService.GetLocalized(sa, x => x.Name, workContext.WorkingLanguage.Id), //sa.GetLocalized(x => x.Name, workContext.WorkingLanguage.Id),
                            SpecificationAttributeDisplayOrder = sa.DisplayOrder,
                            SpecificationAttributeOptionId = sao.Id,
                            SpecificationAttributeOptionName = _localizationService.GetLocalized(sao, x => x.Name, workContext.WorkingLanguage.Id), //sao.GetLocalized(x => x.Name, workContext.WorkingLanguage.Id),
                ...
                if (alreadyFilteredOptions.Count > 0 || notFilteredOptions.Count > 0)
                {
                    ...
                    this.NotFilteredItems = filterByProductAttribute.OrderBy(saof => saof.SpecificationAttributeName)
                        .ThenBy(saof => saof.SpecificationAttributeName)
                        .ThenBy(saof => saof.SpecificationAttributeOptionName).ToList();
                    //------End----
