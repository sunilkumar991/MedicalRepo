...
(
	...
	@FilterableProductIds NVARCHAR(MAX) = null OUTPUT-------Added by Alex on 19-Oct-2018
)
AS
BEGIN
	...
	END
		...
		--filter by no vendor, Added by Alexandar Rajavel on 01-Feb-2019
	IF @VendorId < 0
	BEGIN
		SET @sql = @sql + '
		AND p.VendorId = ' + CAST(0 AS nvarchar(max))
	END
	...
	BEGIN		
		...
		(
			...
			[ProductId] INT NOT NULL ------Added by Alex on 19-Oct-2018
		)
        DECLARE @sql_filterableSpecs nvarchar(max)
        SET @sql_filterableSpecs = '
	        INSERT INTO #FilterableSpecs ([SpecificationAttributeOptionId],[ProductId])
	        SELECT DISTINCT [psam].SpecificationAttributeOptionId,[psam].ProductId
		...
		--Added by Alex on 19-Oct-2018
		SELECT @FilterableProductIds = COALESCE(@FilterableProductIds + ',' , '') + CAST(ProductId AS NVARCHAR(4000))
		FROM #FilterableSpecs
