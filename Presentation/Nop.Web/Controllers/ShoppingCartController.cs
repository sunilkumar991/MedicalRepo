...
namespace Nop.Web.Controllers
{
    public partial class ShoppingCartController : BasePublicController
    {
        ...
        {
            var errors = new ErrorListModel();
            if (updatecartitem == null)
            {
                //add to the cart
                errors = _shoppingCartService.AddToCart(_workContext.CurrentCustomer,
                    product, cartType, _storeContext.CurrentStore.Id,
                    attributes, customerEnteredPriceConverted,
                    rentalStartDate, rentalEndDate, quantity, true);
                addToCartWarnings.AddRange(errors.ErrorList);
            }
            else
            {
            ...
            int quantity, bool forceredirection = false)
        {
            ...
            if (addToCartWarnings.ErrorList.Any())
            {
                ...
                {
                    success = false,
                    message = addToCartWarnings.ErrorList.ToArray()
                });
            }
            ...
            foreach (var sci in pageCart)
            {
                if (allIdsToAdd.Contains(sci.Id))
                {
                    ...
                    if (_shoppingCartSettings.MoveItemsFromWishlistToCart && //settings enabled
                        ...
                        !warnings.ErrorList.Any()) //no warnings ( already in the cart)
                    {
                        //let's remove the item from wishlist
                        _shoppingCartService.DeleteShoppingCartItem(sci);
                    }
                    allWarnings.AddRange(warnings.ErrorList);
                }
            }
