...
namespace Nop.Web.Areas.Admin.Controllers
{
    public partial class CustomerController : BaseAdminController
    {
        ...
        [HttpPost, ActionName("List")]
        [FormValueRequired("ExportAllCustomerList")]
        public virtual IActionResult ExportAllCustomerList(CustomerSearchModel model)
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageCustomers))
                return AccessDeniedView();
            ...
            Sheet.Cells["A1"].Value = "CustomerId";
            Sheet.Cells["B1"].Value = "MobileNumber";
            Sheet.Cells["C1"].Value = "Email";
            Sheet.Cells["D1"].Value = "Active";
            
            //Sheet.Cells["E1"].Value = "FirstName";
            //Sheet.Cells["F1"].Value = "LastName";
            //Sheet.Cells["G1"].Value = "StreetAddress";
            //Sheet.Cells["H1"].Value = "StreetAddress2";
            //Sheet.Cells["I1"].Value = "City";
            ...
            foreach (var item in customers)
            {
                    ...
                    Sheet.Cells[string.Format("D{0}", row)].Value = item.Active;
                    //Sheet.Cells[string.Format("E{0}", row)].Value =(item.BillingAddress==null?"No-Address": (item.BillingAddress.FirstName==null?"No-Name":item.BillingAddress.FirstName));
                    //Sheet.Cells[string.Format("F{0}", row)].Value = (item.BillingAddress == null ? "No-Address" : (item.BillingAddress.LastName == null ? "No-Name" : item.BillingAddress.LastName));
                    //Sheet.Cells[string.Format("G{0}", row)].Value = (item.BillingAddress == null ? "No-Address" : (item.BillingAddress.Address1 == null ? "No-Address1" : item.BillingAddress.Address1));
                    //Sheet.Cells[string.Format("H{0}", row)].Value = (item.BillingAddress == null ? "No-Address" : (item.BillingAddress.Address2 == null ? "No-Address2" : item.BillingAddress.Address2));
                    //Sheet.Cells[string.Format("J{0}", row)].Value = (item.BillingAddress == null ? "No-Address" : (item.BillingAddress.City.Name == null ? "No-City" : item.BillingAddress.City.Name));
                    //Sheet.Cells[string.Format("I{0}", row)].Value = (item.BillingAddress == null ? "No-Address" : (item.BillingAddress.PhoneNumber == null ? "No-PhoneNumber" : item.BillingAddress.PhoneNumber));
                    row++;
            ...
            return File(Ep.GetAsByteArray(), MimeTypes.TextXlsx, "Customers.xlsx");
        ...
        #region Transaction failed list count
        public virtual IActionResult GetTransactionFailedListCount()
        {
            //var transactions = _paymentTransactionHistoryService.GetTransactionFailedCustoemrList();
            //return Json(new { TotalRecordsCount = transactions.Count() });
           // var transactions = _paymentTransactionHistoryService.GetTransactionFailedCustoemrList();
            return Json(new { TotalRecordsCount = 50});
