...
namespace Nop.Web.Areas.Admin.Controllers
{
    public partial class ProductController : BaseAdminController
    {
        ...
        //private readonly IAdminPanelNotificationService _adminPanelNotificationService;
        ...
        public ProductController(IAclService aclService,
            ...
            //IAdminPanelNotificationService adminPanelNotificationService,
             ...
             IScheduleTaskService scheduleTaskService)
        {
            ...
            //_adminPanelNotificationService = adminPanelNotificationService;
        ...
        public virtual IActionResult SyncData(ProductSearchModel searchModel)
        {

            //prepare model
           // var model = _productModelFactory.ProductSearchForSync(searchModel);

            var allproducts = _dbContext.EntityFromSql<Product>("LoadPublishedProducts").ToList();
           // F0B480A3-0A75-42F1-AD6B-A41A30461BAA
            string GETERPApi = configuration.GetValue<string>("ERPApi1:Api1");
            var messageBody = string.Empty;
            int countofsuccess = 0;
            foreach (var product in allproducts)
            {
                if (product.CGMItemID != null)
                {
                    if (product.CGMItemID != "")
                    {
                        messageBody = "[{ \"erpProductId\":\"" + product.CGMItemID + "\"}]";
                        var httpWebRequest = (HttpWebRequest)WebRequest.Create(GETERPApi);
                        ...
                        try
                        {
                            ...
                            {
                                var productnew = _productService.GetProductById(Convert.ToInt32(product.Id));
                                var result = streamReader.ReadToEnd();

                                if (result != "[]")
                                {
                                    result = result.Replace("[", "").Replace("]", "");
                                    ERPResonseHandle bsObj = JsonConvert.DeserializeObject<ERPResonseHandle>(result);
                                    productnew.StockQuantity = bsObj.AvailableQty;
                                    productnew.Price = Convert.ToInt32(bsObj.Rate);
                                    if (bsObj.IsAvailabe)
                                    {
                                        _productService.UpdateProduct(productnew);
                                        countofsuccess++;
                                    }
                                }
                        ...
                        "AdminErrorinSync"), FileMode.Append, FileAccess.Write);
                            StreamWriter objStreamWriter = new StreamWriter(objFilestream);
                            objStreamWriter.WriteLine(errormsg);
            ...
            ViewBag.SuccessCount = countofsuccess;
            ...
            try
            {
                ...
                var scheduleTask = _scheduleTaskService.GetTaskById(4)
        ...
        public class Root
        {
            public ERPResonseHandle ERPResonseHandlelst { get; set; }
        }
        ...
        public virtual IActionResult ProductSpecAttrUpdate(ProductSpecificationAttributeModel model)
        {
            ...
            if (model.AttributeTypeId == (int)SpecificationAttributeType.Option || model.AttributeTypeId == (int)SpecificationAttributeType.CustomText)
            {
        ...
        [HttpPost, ActionName("List")]
        [FormValueRequired("ExportAllProductList")]
        public virtual IActionResult ExportAllProductList(ProductSearchModel model)
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageProducts))
                return AccessDeniedView();
            ...
            Sheet.Cells["A1"].Value = "EcommId";
            Sheet.Cells["B1"].Value = "ERProductCode";
            Sheet.Cells["C1"].Value = "SkuCodeErp";
            Sheet.Cells["D1"].Value = "Barcode";
            Sheet.Cells["E1"].Value = "Name";
            Sheet.Cells["F1"].Value = "Price";
            Sheet.Cells["G1"].Value = "ShortDescription";
            ...
            foreach (var item in products)
            {
                ...
                Sheet.Cells[string.Format("B{0}", row)].Value = item.ERPItemCode;
                Sheet.Cells[string.Format("C{0}", row)].Value = (item.skucodeerp ==null ? "" : item.skucodeerp);
                Sheet.Cells[string.Format("D{0}", row)].Value = item.Barcode;
                Sheet.Cells[string.Format("E{0}", row)].Value = item.Name;
                Sheet.Cells[string.Format("F{0}", row)].Value = item.Price;
                Sheet.Cells[string.Format("G{0}", row)].Value = item.ShortDescription;
                row++;
            }
            ...
            return File(Ep.GetAsByteArray(), MimeTypes.TextXlsx, "products.xlsx");
            ...
            //try
            //{
            //    var bytes = _exportManager.ExportProductsToXlsxNewBySunilKumar(products);
        ...
        [HttpPost]
        public virtual IActionResult importexcelForPrice(IFormFile importexcelfile)
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageProducts))
                return AccessDeniedView();
        ...
        public virtual IActionResult ProductAttributeValueEditPopup(ProductAttributeValueModel model)
        {
            ...
            if (!string.IsNullOrWhiteSpace(model.MeasurementData))
            {
                if (!model.MeasurementData.Contains(@"\n"))
                    ModelState.AddModelError(string.Empty, "Measurement Data needs atleast one \\n");
            }
            ...
            if (ModelState.IsValid)
            {
                ...
                productAttributeValue.MeasurementData = model.MeasurementData;
        ...
        public virtual IActionResult ProductAttributeCombinationEditPopup(ProductAttributeCombinationModel model)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                // Added by Alexandar Rajavel on 23-Feb-2019
                if (previousStockQuantity <= 0 && model.StockQuantity > 0 && !string.IsNullOrEmpty(product.NotificationToCustomers_1))
                {
                    var customersId = product.NotificationToCustomers_1.Split(',');
                    foreach (var id in customersId)
                    {
                        var deviceDetails = _deviceService.GetDevicesByCustomerId(Convert.ToInt32(id));
                        if (deviceDetails.Any())
                    ...
                    product.NotificationToCustomers_1 = string.Empty;
                }
