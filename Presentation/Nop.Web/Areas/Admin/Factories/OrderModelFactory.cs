...
namespace Nop.Web.Areas.Admin.Factories
{
    ...
    public partial class OrderModelFactory : IOrderModelFactory
    {
        ...
        protected virtual void PrepareAddressModel(AddressModel model, Address address)
        {
            ...
            model.IsLiftOptionEnabled = true;
            model.IsLiftOptionRequired = false;
            model.RopeColorEnabled = true;
            model.RopeColorRequired = false;
        ...
        protected virtual void PrepareOrderModelShippingInfo(OrderModel model, Order order)
        {
            ...
            {
                //No action required .. Commented by Ankur on 3 Nov.
        ...
        public virtual OrderSearchModel PrepareOrderSearchModel(OrderSearchModel searchModel)
        {
            ...
            //prepare available billing Cities
            searchModel.AvailableCites = _cityService.GetCitiesByStateId(83, 1, true)
                .Select(city => new SelectListItem { Text = city.Name, Value = city.Id.ToString() }).ToList();
            searchModel.AvailableCites.Insert(0, new SelectListItem { Text = _localizationService.GetResource("Admin.Common.All"), Value = "0" });
        ...
        public virtual OrderListModel PrepareOrderListModel(OrderSearchModel searchModel)
        {
            ...
            //get orders
            var orders = _orderService.SearchOrdersForAdmin(storeId: searchModel.StoreId,
        ...
        public virtual OrderModel PrepareOrderModel(OrderModel model, Order order, bool excludeProperties = false)
        {
            ...
            var DeliveryBoyList = _customerService.GetDeliveryBoyList(order.OrderStatus.ToString());
            List<SelectListItem> listItems = new List<SelectListItem>();
            listItems.Add(new SelectListItem() { Text = "Select Delivery Boy", Value = "0" });
            foreach (var c in DeliveryBoyList)
            {
                string FirstName = _genericAttributeService.GetAttribute<string>(c, Core.Domain.Customers.NopCustomerDefaults.FirstNameAttribute);
                string LastName = _genericAttributeService.GetAttribute<string>(c, Core.Domain.Customers.NopCustomerDefaults.LastNameAttribute);
                listItems.Add(new SelectListItem { Value = c.Id.ToString(), Text = FirstName + " " + LastName });
            }
            model.DeliveryBoyList = listItems;
        ...
        public virtual OrderAddressModel PrepareOrderAddressModel(OrderAddressModel model, Order order, Address address)
        {
            ...
            //Added by Sunil Kumar At 08-03-19
            //prepare available cities
            _baseAdminModelFactory.PrepareCities(model.Address.AvailableCities, model.Address.StateProvinceId);
        ...
        public virtual ShipmentSearchModel PrepareShipmentSearchModel(ShipmentSearchModel searchModel)
        {
            ...
            //Added by Ankur Shrivastava
            _baseAdminModelFactory.PrepareCities(searchModel.AvailableCities, searchModel.StateProvinceId);
