...
namespace Nop.Web.Areas.Admin.Factories
{
    ...
    public partial class BaseAdminModelFactory : IBaseAdminModelFactory
    {
        ...
        public virtual void PrepareVendors(IList<SelectListItem> items, bool withSpecialDefaultItem = true, string defaultItemText = null)
        {
            ...
            items.Add(new SelectListItem { Text = "No vendor", Value = "-1" });// Added by Alexandar Rajavel on 01-Feb-2019
        ...
        /// <param name="withSpecialDefaultItem">Whether to insert the first special item for the default value</param>
        /// <param name="defaultItemText">Default item text; pass null to use default value of the default item text</param>
        public virtual void PrepareCities(IList<SelectListItem> items, int? stateId, bool withSpecialDefaultItem = true, string defaultItemText = null)
        {
            if (items == null)
                throw new ArgumentNullException(nameof(items));
            //Added StateId Validation at 04-11-19 By Sunil Kumar
            if (stateId.HasValue)
            {
                //prepare available cities
                var availableCities = _cityService.GetCitiesByStateId(stateId.Value, 0, false);
                foreach (var c in availableCities)
                {
                    items.Add(new SelectListItem { Value = c.Id.ToString(), Text = c.Name });
                }
            }
            //insert special item for the default value
            PrepareDefaultItem(items, withSpecialDefaultItem, defaultItemText ?? _localizationService.GetResource("Admin.Address.SelectCity"));
        }
        #endregion
    }
}