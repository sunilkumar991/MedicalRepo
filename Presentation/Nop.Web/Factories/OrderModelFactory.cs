...
namespace Nop.Web.Factories
{
    ...
    public partial class OrderModelFactory : IOrderModelFactory
    {
        ...
        public virtual CustomerOrderListModel PrepareCustomerOrderListModel()
        {
            ...
            {
                //Added code By Sunil Kumar at 3/11/18 for Display Latest Order Note that have Permission to Display to Customer 
                string ordernote = "";
                if (order.OrderNotes != null)
                {
                    if (order.OrderNotes.ToList().Count() > 0)
                    {
                        if (order.OrderNotes.ToList()[order.OrderNotes.ToList().Count() - 1].DisplayToCustomer)
                        {
                            ordernote = order.OrderNotes.ToList()[order.OrderNotes.ToList().Count() - 1].Note;
                        }
                    }
                }
                var orderModel = new CustomerOrderListModel.OrderDetailsModel
                {
                    ...
                    OrderNote = ordernote,
                    OrderGroupNumber = Convert.ToString(order.OrderGroupNumber)
                };
                //if (order.PickupAddress != null)
                //{
                //    if (order.PickupAddress.Address1 != null)
                //    {
                //        orderModel.PickUpAddress = order.PickupAddress.Address1;
                //    }
                //    else
                ...
                {
                    orderModel.ExpectedDeliveryDate = Convert.ToDateTime(order.ExpectedDeliveryDate);
                }
        ...
        public virtual OrderDetailsModel PrepareOrderDetailsModel(Order order)
        {
            ...
            if (order.ShippingStatus != ShippingStatus.ShippingNotRequired)
            {
                ...
                if (!order.PickUpInStore)
                {
                    ...
                    model.PickupAddress = new AddressModel
                    {
                        ...
                        City = order.PickupAddress.City?.Name,
            ...
            //PickupAddress info
            _addressModelFactory.PrepareAddressModel(model.PickupAddress,
