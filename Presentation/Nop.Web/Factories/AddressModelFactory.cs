    ...
    public partial class AddressModelFactory : IAddressModelFactory
    {
            ...
            string overrideAttributesXml = "")
        {
            ...
            if (addressSettings.CountryEnabled && loadCountries != null)
            {
                ...
                // Added for City should be selected in dropdown in case editing address -- By Ankur Shrivastava on 27/8/2018
                if (addressSettings.CityEnabled)
                {
                    var languageId = EngineContext.Current.Resolve<IWorkContext>().WorkingLanguage.Id;
                    var cities = _cityService.GetCitiesByStateId(model.StateProvinceId.HasValue ? model.StateProvinceId.Value : 0, languageId)
                        .ToList();
                    if (cities.Any())
                    {
