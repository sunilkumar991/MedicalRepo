...
var PickupPointMethod = {
    ...
    toggleUseRewardPoints: function (useRewardPointsInput) {
        if (useRewardPointsInput.checked) {
            $('#pickup-point-method-block').hide();
        }
        else {
            $('#pickup-point-method-block').show();
        }
    },

    validate: function () {
        var methods = document.getElementsByName('pickuppointmethod');
        if (methods.length == 0) {
            alert('Your order cannot be completed at this time as there is no pick-up point methods available for it.');
            return false;
        }
        var flag = false;
        for (var i = 0; i < methods.length; i++) {
        ...
        var IsDeliveryAllowed = $('#IsDeliveryAllowed').val();
        if (!IsDeliveryAllowed && !flag)
            alert('Please select the pickup point as we not do home delivery in selected township.');
        else
            return true;
        return false;
    },
    ...
    save: function () {
        ...
        if (this.validate()) {
            Checkout.setLoadWaiting('pickup-point-method');
            $.ajax({
                cache: false,
                url: this.saveUrl,
                data: $(this.form).serialize(),
