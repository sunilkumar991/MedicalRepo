...
namespace Nop.Services.Customers
{
    ...
    public partial class CustomerRegistrationService : ICustomerRegistrationService
    {
        ...
        public virtual CustomerLoginResults ValidateCustomer(string usernameOrEmail, string password, string simId = "")
        {
            ...
            if (!PasswordsMatch(_customerService.GetCurrentPassword(customer.Id), password))
            {
                ...
                if (!string.IsNullOrEmpty(simId) && customer.SimId != simId)
                {
        ...
        public virtual CustomerRegistrationResult RegisterCustomer(CustomerRegistrationRequest request)
        {
            ...
            //Remove email validation for testing
            ...
            //Validate unique user email address, remove unique validation of email on 21-Sep-2018 as discussed with Sunil Kumar & Sanakar
        ...
        public virtual void SetIsDisplayEmail(Customer customer, bool IsDisplayEmail)
        {
            if (customer == null)
                throw new ArgumentNullException(nameof(customer));

            customer.IsDisplayEmail = IsDisplayEmail;
            _customerService.UpdateCustomer(customer);
        }
        ...
        /// <param name="password">Password</param>
        /// <returns>Result</returns>
        public virtual void UpdateNewPassword(Customer customer, string password)
        {
            var customerPassword = new CustomerPassword
            {
                Customer = customer,
