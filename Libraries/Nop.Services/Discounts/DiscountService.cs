...
namespace Nop.Services.Discounts
{
    ...
    public partial class DiscountService : IDiscountService
    {
        ...
        /// <summary>
        /// GetDiscountPercentageAndName
        /// </summary>
        /// <param name="discountId">Product Object</param>
        /// <returns>Discount Percentage And Name</returns>
        public virtual Tuple<int, string> GetDiscountPercentageAndName(Product product)
        {
            var percentage = product.DiscountProductMappings.Select(x => new { x.Discount.DiscountPercentage, x.Discount.Name }).ToList();
            var discountStartDateUtc = product.DiscountProductMappings.Select(x => new { x.Discount.StartDateUtc }).ToList();
            var discountEndDateUtc = product.DiscountProductMappings.Select(x => new { x.Discount.EndDateUtc }).ToList();
            DateTime now = DateTime.Now;
            var currentDateTimeUtc = now;
            var DiscountPercentage = 0;
            var DiscountOfferName = string.Empty;
            if (percentage.Count > 0 && discountStartDateUtc.Count > 0 && discountEndDateUtc.Count > 0)
            {
                if ((currentDateTimeUtc >= discountStartDateUtc[0].StartDateUtc) && (currentDateTimeUtc <= discountEndDateUtc[0].EndDateUtc))
                {
                    DiscountPercentage = Convert.ToInt32(percentage[0].DiscountPercentage);
                    DiscountOfferName = percentage[0].Name;
                }
            }
            return new Tuple<int, string>(DiscountPercentage, DiscountOfferName);
        }
