...
namespace Nop.Services.Shipping
{
    ...
    public partial class ShipmentService : IShipmentService
    {
            ...
            int pageIndex = 0, int pageSize = int.MaxValue)
        {
            ...
            if (shippingCityId > 0)
                query = query.Where(s => s.Order.ShippingAddress.CityId == shippingCityId);

            if (!string.IsNullOrWhiteSpace(shippingCounty))
                query = query.Where(s => s.Order.ShippingAddress.County.Contains(shippingCounty));
            //if (shippingCityId > 0)
            //    query = query.Where(s => s.Order.ShippingAddress.CityId.Contains(shippingCity));
            if (loadNotShipped)
