...
namespace Nop.Services.Orders
{
    ...
    public partial class OrderTotalCalculationService : IOrderTotalCalculationService
    {
            ...
            out decimal taxRate, out List<DiscountForCaching> appliedDiscounts)
        {
            ...
            ////added code by Sunil Kumar at 11-04-2020 for shipping charge apply only  6 PM to 11.59 PM start
            //DateTime now = DateTime.Now;
            //TimeSpan start = TimeSpan.Parse("18:00"); // 6 PM
            //TimeSpan end = TimeSpan.Parse("23:59");   // 11.59 PM
            ////Added time By Sunil Kumar At 13-04-2020
            //TimeSpan orderTime = now.AddHours(6.30).TimeOfDay;
            ...
            //Commented by Sunil Kumar on 09082021 
            //added code by Sunil Kumar at 11-04-2020 for shipping charge apply only  6 PM to 11.59 PM end

            if (isFreeShipping)
                return decimal.Zero;

            // Added by Alexandar Rajavel on 17-Nov-2018, updated on 10-June-2019
            //shippingTotal = cart.Sum(p => p.Product.AdditionalShippingCharge * p.Quantity);
            // start
            var shippingCharges = 0M;
            //var shippingChargeBaseAmount = cart[0].Customer.BillingAddress == null ? shippingCharges : cart[0].Customer.BillingAddress.City.ShippingCharge ?? shippingCharges;
            var city = cart[0].Customer.BillingAddress == null ? null : cart[0].Customer.BillingAddress.City;
            if (city != null && !string.IsNullOrEmpty(cart[0].Customer.BillingAddress.Address1))
            {
                foreach (var item in cart)
                {
                    if (item.Product.IsShipEnabled)
                    {
                        shippingCharges = cart[0].Customer.BillingAddress == null ? shippingCharges : cart[0].Customer.BillingAddress.City.ShippingCharge ?? shippingCharges;
                    }
                    //switch (decimal.Ceiling(item.Product.Weight))
                    //{
                    //    case 0:
                    //    case 1:
                    //        shippingCharges += _shippingSettings.IsShippingChargeQtyWise ? (city.ShippingChargeUptoOneKg ?? 0M) * item.Quantity : (city.ShippingChargeUptoOneKg ?? 0M);
                    //        break;
                    //    case 2:
                    //        shippingCharges += _shippingSettings.IsShippingChargeQtyWise ? (city.ShippingChargeUptoTwoKg ?? 0M) * item.Quantity : (city.ShippingChargeUptoTwoKg ?? 0M);
                    //        break;
                    //    case 3:
                    //        shippingCharges += _shippingSettings.IsShippingChargeQtyWise ? (city.ShippingChargeUptoThreeKg ?? 0M) * item.Quantity : (city.ShippingChargeUptoThreeKg ?? 0M);
                    //        break;
                    //    case 4:
                    //        shippingCharges += _shippingSettings.IsShippingChargeQtyWise ? (city.ShippingChargeUptoFourKg ?? 0M) * item.Quantity : (city.ShippingChargeUptoFourKg ?? 0M);
                    //        break;
                    //    case 5:
                    //        shippingCharges += _shippingSettings.IsShippingChargeQtyWise ? (city.ShippingChargeUptoFiveKg ?? 0M) * item.Quantity : (city.ShippingChargeUptoFiveKg ?? 0M);
                    //        break;
                    //    default:
                    //        shippingCharges += _shippingSettings.IsShippingChargeQtyWise ? ((city.ShippingChargeUptoFiveKg ?? 0M) + city.ShippingChargeAboveFiveKg ?? 0M) * item.Quantity : ((city.ShippingChargeUptoFiveKg ?? 0M) + city.ShippingChargeAboveFiveKg ?? 0M);
                    //        break;
                    //}
                }
            }
            ...
            else if (!shippingTotal.HasValue)
            {
        ...
        public virtual decimal? GetShoppingCartTotal(IList<ShoppingCartItem> cart,
            bool? useRewardPoints = null, bool usePaymentMethodAdditionalFee = true, bool useInbuildDiscount = true)
        {
            return GetShoppingCartTotal(cart, out _, out _, out _, out _, out _, useRewardPoints, usePaymentMethodAdditionalFee, useInbuildDiscount);
        }
            ...
            bool? useRewardPoints = null, bool usePaymentMethodAdditionalFee = true, bool useInbuildDiscount = true)
        {
            ...
            var customer = cart.FirstOrDefault(item => item.Customer != null)?.Customer;
            var paymentMethodSystemName = string.Empty;
            var subpaymenttype = string.Empty;
            if (customer != null)
            {
                ...
                subpaymenttype = _genericAttributeService.GetAttribute<string>(customer,
            ...
            // Commented by Sunil Kumar for avoiding shipping charges adding twice in product total

            //var shoppingCartShipping = decimal.Zero;
            ...
            if (paymentMethodSystemName != string.Empty && subpaymenttype=="VISA" && useInbuildDiscount)
            {
                discountAmount = 0;
                ////if ordertotal is >20000 and its discount(10%)>3000 = 3000  else  10% discount only
                //if (orderTotal > 20000)
                //{
                //    var appliedDiscount = (orderTotal * (decimal)0.1) > 3000 ? 3000 : (orderTotal * (decimal)0.1);
                //    orderTotal = orderTotal - appliedDiscount;
