...
namespace Nop.Services.Orders
{
    ...
    public partial class OrderService : IOrderService
    {
        ...
        public OrderService(IEventPublisher eventPublisher, //ILocalizationService localizationService, INotificationService notificationService, IDeviceService deviceService,
        ...
        public virtual Order GetOrderByCustomOrderNumber(string customOrderNumber)
        {
            ...
            //var records = _orderRepository.Table.Where(o => o.CustomOrderNumber == customOrderNumber);
        ...
        /// </summary>
        /// <param name="deliveryBoyId">The delivery Boy Id</param>
        /// <returns>Order</returns>
        public virtual IList<Order> GetOrderListByDeliveryBoyId(int deliveryBoyId)
        {
            if (deliveryBoyId == 0)
                return null;
            var query = from o in _orderRepository.Table
                            // Where conditions changed by Alexandar Rajavel on 19-01-2019
                            //where o.DeliveryBoyId == deliveryBoyId && !o.Deleted && o.PaymentStatus == PaymentStatus.Paid && !o.PickUpInStore && o.ShippingStatus != ShippingStatus.Delivered && o.IsDeliveryAccepted
                            //Added code By Sunil Kumar at 26-03-2020 for listing delivery Boy Order Only using Id
                        where !o.Deleted && o.DeliveryBoyId == deliveryBoyId && !o.PickUpInStore && o.ShippingStatus != ShippingStatus.Delivered
                        select o;
        ...
        /// <summary>
        /// Created By: Alexandar Rajavel on 08-July-2019
        /// </summary>
        /// <param name="deliveryBoyId">The delivery Boy Id</param>
        /// <returns>Order</returns>
        public virtual IList<Order> GetMyDeliveredList(int deliveryBoyId)
        {
            if (deliveryBoyId == 0)
                return null;
            var query = from o in _orderRepository.Table
                        where o.DeliveryBoyId == deliveryBoyId && !o.Deleted && o.ShippingStatus == ShippingStatus.Delivered
                        select o;
            var Orders = query.ToList();
            return Orders;
        }
        ...
        /// <summary>
        /// Created By: Alexandar Rajavel on 22-Jan-2019
        /// </summary>
        /// <param name="customOrderNumber">The custom order number</param>
        /// <returns>Order</returns>
        public virtual Order GetOrderByCustomOrderNumberForDelivery(string customOrderNumber)
        {
            if (string.IsNullOrEmpty(customOrderNumber))
                return null;
            var records = _orderRepository.Table.Where(o => o.CustomOrderNumber == customOrderNumber && !o.Deleted && o.PaymentStatus == PaymentStatus.Paid && !o.PickUpInStore && o.ShippingStatus != ShippingStatus.Delivered && !o.IsDeliveryAccepted);
            return records.FirstOrDefault(o => o.CustomOrderNumber == customOrderNumber);
        }
        ...
        /// <summary>
        /// Created by Alexandar Rajavel on 06-04-2019
        /// Gets an order
        /// </summary>
        /// <param name="trasactionId">The customer transaction number</param>
        /// <returns>Order</returns>
        public virtual List<Order> GetOrdersByTransactionId(string trasactionId)
        {
            ...
            return _orderRepository.Table.Where(o => o.AuthorizationTransactionId == trasactionId).ToList();
        }
        ...
        /// <param name="customOrderNumber">Search in customOrderNumber. Leave empty to load all records.</param>
        /// <param name="pageIndex">Page index</param>
        /// <param name="pageSize">Page size</param>
        /// <param name="getOnlyTotalCount">A value in indicating whether you want to load only total number of records. Set to "true" if you don't want to load data from database</param>
        /// <returns>Orders</returns>
        public virtual IPagedList<Order> SearchOrdersForAdmin(int storeId = 0,
            ...
            string orderNotes = null, string customOrderNumber = null, int pageIndex = 0, int pageSize = int.MaxValue, bool getOnlyTotalCount = false, string deviceId = null)
        {
            var query = _orderRepository.Table;
            //query = query.Where(o => o.PaymentStatusId != (int)PaymentStatus.Pending); // Show only payment paid orders to admin by ankur on 24 Sep. 2019
            if (storeId > 0)
            ...
            if (billingCountryId > 0)
                query = query.Where(o => o.BillingAddress != null && o.BillingAddress.CityId == billingCountryId);
            ...
            //added by Sunil at 09-04-19
            if (!string.IsNullOrEmpty(customOrderNumber))
            {
                var records = _orderRepository.Table.Where(o => o.CustomOrderNumber == customOrderNumber && !o.Deleted);
                if (records.Count() > 0)
                {
                    var AuthorizationTransactionId = records.FirstOrDefault(o => o.CustomOrderNumber == customOrderNumber).AuthorizationTransactionId;
                    query = query.Where(o => o.AuthorizationTransactionId == AuthorizationTransactionId);
                }
            }
            //added by Sunil at 24-04-2020 for Device Id Filter
            if (!string.IsNullOrEmpty(deviceId))
            {
                //added by Sunil at 25-04-2020 for Device Id Filter some extra condition
                var records = _orderRepository.Table.Where(o => o.DeviceId == deviceId && !o.Deleted);
                if (records.Count() > 0)
                {
                    query = query.Where(o => o.DeviceId == deviceId|| o.DeviceId == null || o.DeviceId == string.Empty);
                }
                else
                {
                    query = query.Where(o => o.DeviceId == null || o.DeviceId == string.Empty);
                }
            }
        ...
        public virtual void InsertOrder(Order order)
        {
            if (order == null)
                throw new ArgumentNullException(nameof(order));
            //Six digit code implementation
            Random generator = new Random();
            order.DeliveryCode = generator.Next(0, 1000000).ToString("D6");
        ...
        public virtual void UpdateOrder(Order order)
        {
            ...
            //var devicedetails = _deviceService.GetDevicesByCustomerId((int)order.DeliveryBoyId);
        ...
        /// <returns></returns>
        public virtual int GetMaxOrderGroupNumber(int CustomerId)
        {
            int groupNumber = 0;
            var query = _orderRepository.Table.Where(o => o.CustomerId == CustomerId).OrderByDescending(o => o.OrderGroupNumber).FirstOrDefault();
            return query != null ? query.OrderGroupNumber : groupNumber;
        ...
        /// Added By Ankur for EC-151
        /// </summary>
        /// <param name="OrderGroupNumber"></param>
        /// <returns></returns>
        public virtual List<Order> GetOrdersByOrderGroupNumber(int CustomerId, int OrderGroupNumber)
        {
            var query = _orderRepository.Table.Where(o => o.CustomerId == CustomerId && o.OrderGroupNumber == OrderGroupNumber).ToList();
            return query;
        }
        ...
        public IPagedList<Order> SearchOrdersForPickUp1(int storeId = 0, int vendorId = 0, int customerId = 0, int productId = 0, int affiliateId = 0, int warehouseId = 0, int billingCountryId = 0, string paymentMethodSystemName = null, DateTime? createdFromUtc = null, DateTime? createdToUtc = null, List<int> osIds = null, List<int> psIds = null, List<int> ssIds = null, string billingPhone = null, string billingEmail = null, string billingLastName = "", string orderNotes = null, string customOrderNumber = null, int pageIndex = 0, int pageSize = int.MaxValue, bool getOnlyTotalCount = false, string deviceId = null, DateTime? editDateTime = null, DateTime? editEndDate = null)
        {
            var query = _orderRepository.Table;
            //query = query.Where(o => o.PaymentStatusId != (int)PaymentStatus.Pending); // Show only payment paid orders to admin by ankur on 24 Sep. 2019
            if (storeId > 0)
