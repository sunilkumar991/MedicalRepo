...
namespace Nop.Services.Catalog
{
    ...
    public partial class PriceFormatter : IPriceFormatter
    {
            ...
            bool showCurrency, Currency targetCurrency)
        {
            ...
            long amount = (long)(Amount); //need int value for D format By Ankur Shrivastava on 9 Nov. 2018  #EC-209
            //string price = string.Format("{0:C}", amount);
            string strAmount = string.Format("{0:#,#}", amount);
            if ((strAmount.Trim() == ""))//|| (strAmount == null)
            {
                strAmount = "0";
            }
            ...
            else
            {
                ...
                {
                    result = $"{strAmount:D} {targetCurrency.CurrencyCode}"; //Changed the Amount format to D from N to display integer amount only By Ankur Shrivastava on 9 Nov. 2018 
                    //Use Currency Code changed By Ankur Shrivastava on 11 Sep. 2018 for MMK instead of K
                    //default behavior
                ...
                else
                {
                    ...
                    result = $"{strAmount:D} {targetCurrency.CurrencyCode}";
                    return result;
                }
            }
