...
namespace Nop.Services.Catalog
{
    ...
    public partial class SpecificationAttributeService : ISpecificationAttributeService
    {
        ...
        public virtual IPagedList<SpecificationAttribute> GetSpecificationAttributes(string attributeName = "", int pageIndex = 0, int pageSize = int.MaxValue)
        {
            ...
            {
                query = query.Where(x => x.Name.Contains(attributeName));
            }
        ...
        /// <summary>
        /// Check specification attribute already excist
        /// </summary>
        /// <param name="specificationAttribute">Specification attribute</param>
        public virtual bool CheckSpecificationAttributeAlreadyExcist(string attribute)
        {
            if (attribute == null)
            ...
            {
                query = query.Where(x => x.Name == attribute);
            }
            query = query.OrderBy(x => x.Name);
        ...
        /// <summary>
        /// This is using to get product's specification attribute option ids by using product ids
        /// Created By: Alexandar Rajavel
        /// Created On: 09-Oct-2018
        /// </summary>
        /// <param name="productIds">The specification attribute option identifier</param>
        /// <returns>Ids</returns>
        public virtual List<int> GetProductSpecificationAttributeOptionIdsByProductIds(int[] productIds)
        {
            if (productIds == null || productIds.Length == 0)
                return new List<int>();
            var query = from psa in _productSpecificationAttributeRepository.Table
                        where productIds.Contains(psa.Id)
                        select psa;

            return query.Select(s => s.SpecificationAttributeOptionId).Distinct().ToList();
        }
