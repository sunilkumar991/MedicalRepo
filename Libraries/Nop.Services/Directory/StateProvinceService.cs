    ...
    public partial class StateProvinceService : IStateProvinceService
    {
        ...
        public virtual IList<StateProvince> GetStateProvincesByCountryId(int countryId, int languageId = 0, bool showHidden = false)
        {
            ...
            return _cacheManager.Get(key, () =>
            {
                ...
                stateProvinces = stateProvinces.Any() ? stateProvinces.OrderBy(s => s.Name).ToList() : stateProvinces;
        ...
        /// <returns></returns>
        public virtual StateProvince GetStateByCountryIdAndStateName(int countryId, string stateName)
        {
            if (countryId == 0)
                return null;

            return _stateProvinceRepository.Table.Where(c => c.CountryId == countryId && c.Name.ToLower().Contains(stateName.ToLower().Trim())).FirstOrDefault();
        }
